---
title: 'Từ LoRA đến LongLoRA - Nhẹ hơn, dài hơn và sướng hơn'
date: '2024-02-05'
lastmod: '2023-02-05'
...
W
$
của model sẽ biến thành updated weights $
W'
$
dựa trên giá trị weight cần thay đổi $
\Delta W
$
thu được từ quá trình back propagation. Và ở iteration tiếp theo, $
W'
$
lại được update với một $
\Delta W
$
khác.
...
$
* Iteration 2: $
y = W''x = (W + \Delta W)x = (W' + \Delta W')x = (W + \Delta W + \Delta W')x
$
...
Mục đích của LoRA là tìm cách biểu diễn ma trận $
\Delta W
$
thành một dạng biểu diễn nhẹ hơn. Năm 2020, có một paper nói rằng những mô hình ngôn ngữ pre-trained có intrinsic dimension (hay intrinsic rank) cực kì thấp, tức là, model này có thể được biểu diễn sử dụng số chiều ít hơn rất nhiều so với số chiều gốc của model, mà vẫn giữ được performance khi đem đi fine-tune.

Tận dụng ý tưởng này, nhóm tác giả của LoRA cũng cho rằng, $
\Delta W
$
cũng có thể được biểu diễn với số chiều ít hơn rất nhiều số chiều gốc của $
\Delta W
$
. LoRA chọn sử dụng Matrix decomposition để biểu diễn ma trận $
\Delta W
$
bằng tích của các ma trận con với độ nặng tính toán thấp hơn việc tính trên ma trận gốc. Có rất nhiều phương pháp Matrix decomposition (LU decomposition, Singular Value Decomposition, ...), và LoRA chọn sử dụng phương pháp Neural Network.
...
B
$
với rank thấp hơn rất nhiều so với ma trận ban đầu. Cụ thể, $
\Delta W = BA
$
với $
\Delta W \in \mathrm{R}^{d \times k}, B \in \mathrm{R}^{d \times r}, A \in \mathrm{R}^{r \times k}
$
và số rank $
r \ll \min (d,k)
$
. Lúc này, output của layer đó sẽ trở thành 
$$
y = W_{0}x + \Delta Wx = W_{0}x + BAx
$$
Ma trận A được khởi tạo theo Gaussian initialization, còn B thì được khởi tạo là ma trận 0, nên $
\Delta W = BA
$
có giá trị bằng 0 khi bắt đầu training. Quá trình training sẽ tối ưu đê tính được 2 ma trận cần tìm.

Vậy tại sao việc phân tách này lại làm khối lượng tính toán nhẹ đi? Giả sử ta chọn $
A = B = 100, r = 4
$
. Khi đó, ta có số phần tử của ma trận $
\Delta W
$
là $
100 \times 100 = 10000
$
, còn số phân tử sau phân rã là $
100 \times 4 + 100 \times 4 = 800
$
, tức là khối lượng được giảm đi 12.5 lần.

Để có thể phân tách được một layer Linear, LoRA thêm vào Linear layer đó 2 lớp Linear nữa, mỗi lớp Linear đại diện cho ma trận $
A
$ và $
B
$
. Lúc này nó giống như việc training một NN bình thường thôi. Chú ý là, hiện tại LoRA mới chỉ hỗ trợ phân tách weight của layer Linear, và chưa hỗ trợ cho những layer khác.
